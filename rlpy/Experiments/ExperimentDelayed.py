...
class ExperimentDelayed(object):
    ...
    ##NOTE: Modification is made to make new deepcopy of map every call
    def performanceRun(self, total_steps, visualize=False):
        ...
        while not eps_term and eps_length < self.domain.episodeCap:
            ...
            # print "Current: {0}; Action: {1}; Next: {2}, Reward: {3}".format(s, a, ns, r)
    ...
    def run(self, visualize_performance=0, visualize_learning=False,
        ...
        while total_steps < self.max_steps:
            if terminal or eps_steps >= self.domain.episodeCap:
                ...
                print "Current episode {0}".format(episode_number)
                print "Steps in current episode: {0}".format(eps_steps)
                print "Current episode return {0}".format(eps_return) 
                print "Total steps so far: {0}".format(total_steps)
                print "Terminal state? {0}".format(terminal)

                self.trials['return'].append(eps_return)
                self.trials['steps'].append(total_steps)
                ...
                # passed
                if episode_number and episode_number % 5 == 0 :                
            ...
            if (terminal or eps_steps == self.domain.episodeCap): # and deltaT(start_log_time) > self.log_interval: < Commented out because was preventing any
    ...
    def plot(self, y="return", x="learning_steps", save=False):
        ...
        if save:
            path = os.path.join(
                self.full_path,
                "performance{}.png".format(self.exp_id))
            performance_fig.savefig(path, transparent=True, pad_inches=.1)
        plt.ioff()
        plt.show()

    def plotTrials(self, y="return", x="steps", save=False):
        trials = self.trials
        trial_fig = plt.figure("Trials")
        plt.plot(trials[x], trials[y], '-bo', lw=3, markersize=10)
        if save:
            path = os.path.join(
                self.full_path,
                "performanceTrials{}.png".format(self.exp_id))
            trial_fig.savefig(path, transparent=True, pad_inches=.1)
        plt.ioff()
        plt.show()
